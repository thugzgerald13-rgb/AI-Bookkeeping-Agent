...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(page_title="AI Books", page_icon="📗", layout="wide",
...
#MainMenu,footer,header,[data-testid="manage-app-button"],
...
[data-testid="stSidebar"] .stRadio label:hover{background:rgba(255,255,255,.1)!important;color:#fff!important}
...
.kpi-card{background:white;border-radius:8px;padding:18px 20px;box-shadow:0 1px 4px rgba(0,0,0,.08);border-left:4px solid #2ecc71;margin-bottom:4px}
.kpi-card.red{border-left-color:#e74c3c}.kpi-card.blue{border-left-color:#3498db}
.kpi-card.orange{border-left-color:#e67e22}.kpi-card.teal{border-left-color:#1abc9c}
.kpi-label{font-size:12px;color:#7f8c8d;font-weight:600;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px}
.kpi-value{font-size:24px;font-weight:700;color:#2c3e50}
.kpi-sub{font-size:12px;color:#95a5a6;margin-top:4px}
.section-card{background:white;border-radius:8px;padding:20px 24px;box-shadow:0 1px 4px rgba(0,0,0,.08);margin-bottom:16px}
...
.user-chip{background:rgba(255,255,255,.12);border-radius:20px;padding:6px 12px;font-size:12px;color:#ecf0f1;margin-bottom:12px}
</style>
...
if "db" not in st.session_state:
    ...
    st.session_state.db = SupabaseDB(access_token=token)
...
company = profile.get("company_name", "My Business") if profile else "My Business"
user_email = user.email if user else ""

with st.sidebar:
    st.markdown(f'<div style="padding:8px 0 16px;border-bottom:1px solid rgba(255,255,255,.15);margin-bottom:16px"><div style="font-size:20px;font-weight:800;color:#2ecc71">📗 AI Books</div><div style="font-size:12px;color:#95a5a6;margin-top:2px">{company}</div></div>', unsafe_allow_html=True)

    page = st.radio("", [
        "🏠  Dashboard", "📄  Invoices", "💸  Expenses",
        "🏦  Banking",   "👤  Customers", "🏪  Vendors",
        "📊  Reports",   "🗂️  Chart of Accounts", "💬  AI Advisor",
        "⚙️  Settings",
    ...
    st.divider()
    st.markdown(f'<div class="user-chip">👤 {user_email}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
if page == "🏠  Dashboard":
    page_header("🏠", f"Welcome back, {profile.get('full_name','') if profile else ''} 👋")
    summary = db.get_summary()
    inv_sum = db.get_invoice_summary()
    banks   = db.get_bank_accounts()
    bank_total = sum(float(b["balance"]) for b in banks)
    net = summary["net"]
    ...
    c1,c2,c3 = st.columns(3)
    with c1: kpi("📄 A/R Open",  peso(inv_sum["open"]),    f"{inv_sum['count_open']} invoices", "blue")
...
# ══════════════════════════════════════════════════════════════════════════════
        ...
        if invoices:
            for inv in invoices:
                if inv["status"]=="open" and overdue_check(inv["due_date"]):
                    db._q("invoices").update({"status":"overdue"}).eq("id",inv["id"]).execute()
            invoices = db.get_invoices()
            ...
            for col in ["Total","Paid","Balance Due"]:
                df_show[col] = df_show[col].apply(lambda x: f"₱{float(x):,.2f}")
            st.dataframe(df_show, use_container_width=True, hide_index=True)

            st.markdown("**Record Payment**")
            open_inv = {f"{i['invoice_number']} — {i['customer_name']} (₱{float(i['balance_due']):,.2f})": i["id"]
                        for i in invoices if i["status"] in ("open","overdue","partial")}
            if open_inv:
                c1,c2,c3 = st.columns([2,1,1])
                with c1: sel = st.selectbox("Select Invoice", list(open_inv.keys()), key="pay_inv")
                with c2: pay_amt = st.number_input("Amount (₱)", min_value=0.01, step=100.0, key="pay_amt")
                with c3:
                    st.markdown("<div style='margin-top:28px'></div>", unsafe_allow_html=True)
                    if st.button("✅ Record Payment", type="primary"):
                        db.mark_invoice_paid(open_inv[sel], pay_amt)
                        db.insert_transaction(date=date.today().strftime("%Y-%m-%d"),
                            type_="income", amount=pay_amt, category="Service Revenue",
                            description=f"Payment — {sel.split('—')[0].strip()}")
    ...
    with tab2:
        ...
        if not customers:
            st.markdown('<div class="alert-box alert-warning">⚠️ Add a customer first (Customers page).</div>', unsafe_allow_html=True)
        else:
            with st.form("new_invoice", clear_on_submit=True):
                c1,c2,c3 = st.columns(3)
                cust_map = {c["name"]: c["id"] for c in customers}
                with c1: cust = st.selectbox("Customer *", list(cust_map.keys()))
                ...
                for i in range(1,6):
                    ...
                    with c1: desc  = st.text_input(f"Description {i}", key=f"desc_{i}")
                ...
                st.markdown(f'<div style="background:#f8f9fa;border-radius:8px;padding:12px 16px;margin-top:8px"><div style="display:flex;justify-content:space-between"><span>Subtotal</span><strong>{peso(subtotal)}</strong></div><div style="display:flex;justify-content:space-between;color:#7f8c8d"><span>VAT ({tax_rate}%)</span><strong>{peso(tax)}</strong></div><hr style="border-color:#dee2e6;margin:8px 0"><div style="display:flex;justify-content:space-between;font-size:16px"><strong>Total</strong><strong style="color:#27ae60">{peso(subtotal+tax)}</strong></div></div>', unsafe_allow_html=True)
                if st.form_submit_button("💾 Create Invoice", type="primary", use_container_width=True):
                    if not items: st.error("Add at least one line item.")
                        ...
                        inv_id = db.create_invoice(customer_id=cust_map[cust],
                            issue_date=issue_date, due_date=due_date,
                            items=items, tax_rate=tax_rate, notes=notes)
                        st.success(f"✅ Invoice created!"); st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        txns = db.fetch_by_type("expense", 200)
        if txns:
            df = pd.DataFrame(txns)
            df["amount"] = df["amount"].apply(lambda x: f"₱{float(x):,.2f}")
            show = ["id","date","description","category","amount","reference","payee","ai_notes"]
            st.dataframe(df[[c for c in show if c in df.columns]], use_container_width=True, hide_index=True)
            total_exp = sum(float(t.get("amount","0").replace("₱","").replace(",","")) for t in txns)
            st.markdown(f'<div class="alert-box alert-info">Total Expenses: <strong>₱{total_exp:,.2f}</strong></div>', unsafe_allow_html=True)
            buf = io.StringIO(); w = csv.DictWriter(buf, fieldnames=txns[0].keys())
    ...
    with tab2:
        ...
        cat_opts = [a["name"] for a in accounts if a["type"]=="Expense"] or [
            "Salaries Expense","Rent Expense","Utilities Expense","Office Supplies",
            "Marketing & Advertising","Professional Fees","Transportation",
            "Meals & Entertainment","Taxes & Licenses","Miscellaneous Expense"]
        ...
        with st.form("new_expense", clear_on_submit=True):
            ...
            c1,c2 = st.columns(2)
            with c1: description = st.text_input("Description *")
            ...
            if st.form_submit_button("💾 Save Expense", type="primary", use_container_width=True):
                if not description: st.error("Description required.")
                    ...
                    db.insert_transaction(date=exp_date, type_="expense", amount=amount,
                        category=final_cat, description=description, reference=reference,
                        payee=payee if payee!="— None —" else "", ai_notes=ai_notes)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if banks:
        ...
        for i,bank in enumerate(banks):
            with cols[i%3]:
                color = "#27ae60" if float(bank["balance"])>=0 else "#e74c3c"
    ...
    with tab1:
        ...
        with c2: limit = st.selectbox("Show", [50,100,200,500], index=0)
        txns = db.get_transactions(limit)
        if type_filter: txns = [t for t in txns if t["type"] in type_filter]
        if txns:
            df = pd.DataFrame(txns)
            show_c = ["id","date","type","amount","category","description","reference","payee"]
            df_s = df[[c for c in show_c if c in df.columns]].copy()
            df_s["amount"] = df_s["amount"].apply(lambda x: f"₱{float(x):,.2f}")
        ...
        with st.form("add_txn", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Save", type="primary", use_container_width=True):
                if not txn_desc: st.error("Description required.")
                    ...
                    db.insert_transaction(date=txn_date, type_=txn_type, amount=txn_amt,
                        category=txn_cat, description=txn_desc, reference=txn_ref, payee=txn_payee)
                    if banks:
    ...
    with tab3:
        with st.form("add_bank", clear_on_submit=True):
            ...
            with c2: bbank = st.text_input("Bank Name")
            c1,c2,c3 = st.columns(3)
            ...
            if st.form_submit_button("Add Account", type="primary"):
                if not bname: st.error("Name required.")
                    ...
                    db._q("bank_accounts").insert({"name":bname,"type":btype,"bank_name":bbank,"account_number":bacct,"balance":bbal,"currency":"PHP"}).execute()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        if customers:
            ...
            show = ["id","name","company","email","phone","balance"]
            st.dataframe(df[[c for c in show if c in df.columns]], use_container_width=True, hide_index=True)
            st.markdown(f'<div class="alert-box alert-info">{len(customers)} customers</div>', unsafe_allow_html=True)
        else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        income_cats, expense_cats = db.get_pl_data(str(pl_from), str(pl_to))
        total_inc = sum(income_cats.values())
        ...
        with c1:
            ...
            for cat,amt in sorted(income_cats.items(), key=lambda x:-x[1]):
                st.markdown(f'<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #f4f5f7"><span>{cat}</span><strong style="color:#27ae60">{peso(amt)}</strong></div>', unsafe_allow_html=True)
        ...
        with c2:
            ...
            for cat,amt in sorted(expense_cats.items(), key=lambda x:-x[1]):
                st.markdown(f'<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #f4f5f7"><span>{cat}</span><strong style="color:#e74c3c">{peso(amt)}</strong></div>', unsafe_allow_html=True)
        ...
        if open_inv:
            ...
            for inv in open_inv:
                try:
                    ...
                    days = (today-due).days
                    inv["days_past"] = days
                    inv["bucket"] = "Current" if days<=0 else "1-30" if days<=30 else "31-60" if days<=60 else "61-90" if days<=90 else "90+"
            ...
            df_ar["Balance Due"] = df_ar["Balance Due"].apply(lambda x: f"₱{float(x):,.2f}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("add_acct", clear_on_submit=True):
        c1,c2,c3,c4 = st.columns(4)
        with c1: acode = st.text_input("Code")
        ...
        if st.form_submit_button("Add Account", type="primary"):
            if not aname: st.error("Name required.")
                ...
                db._q("accounts").insert({"code":acode,"name":aname,"type":atype,"subtype":asub}).execute()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        st.markdown('<div class="alert-box alert-info">💡 <strong>Try:</strong> "What are my top expenses?" · "Am I profitable?" · "How to handle BIR VAT?" · "Which customers owe me the most?"</div>', unsafe_allow_html=True)
    st.divider()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("settings_form"):
        ...
        c1,c2 = st.columns(2)
        with c1:
            company_name = st.text_input("Business Name", value=profile.get("company_name","") if profile else "")
            full_name    = st.text_input("Your Name",     value=profile.get("full_name","") if profile else "")
        with c2:
            currency = st.selectbox("Currency", ["PHP","USD","SGD","EUR"], index=0)
            st.text_input("Email", value=user_email, disabled=True)
        if st.form_submit_button("💾 Save Settings", type="primary"):
            db.update_profile(company_name, currency)
            db._q("profiles").update({"full_name":full_name}).eq("id", user.id if user else "").execute()
            st.success("✅ Settings saved!"); st.rerun()

    st.divider()
    st.markdown("**Account**")
    st.markdown(f'<div class="alert-box alert-info">Signed in as <strong>{user_email}</strong></div>', unsafe_allow_html=True)
